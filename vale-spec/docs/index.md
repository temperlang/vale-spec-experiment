...
!!! warning
    This document is a living specification. Do not treat it as the complete implementation; instead use it to drive the next round of AI-assisted and human-reviewed iterations.

## AI-assisted implementation plan

1. Identify gaps and assumptions
    - Review the document and list explicitly every term and behavior that is referenced but not fully defined.
    - Generate a short "open questions" section with context and priority (e.g., security, access control, storage compatibility).
    - Human reviewers validate and refine this list.

2. Generate domain model skeletons
    - Instruct AI to produce language-agnostic schema drafts for every mentioned entity (User, Person, Group, Library, Testimonial, Version, PersonProfile, GroupProfile, LibraryProfile, UserDisplayPreferences, Guarded<T>, Delta types, Ken<T>, Pii<T>). 
    - Include types, optionality, and semantics for access control and PII.
    - Human review and accept as a basis or adjust.

3. Create interface/contract specification
    - Generate interface/class definitions and API endpoints from the domain model in the target language stack with stubs.
    - Use the prose storage abstraction section (from this document) to derive an explicit interface (e.g., StorageBackend, UserContext, GuardedDelta merge).
    - Have AI propose a test harness for storage abstraction compatibility and caching rules.

4. Define behavior flows and tests
    - Produce interaction diagrams and sequence descriptions for key flows (login, group membership updates, library version publish, /pub path rendering, PII gating).
    - Generate test scenarios for both unit and integration (e.g., unprivileged user sees public page, member sees group-only profile fields, delta merge unknown semantics, PII redaction on dump).
    - Human review and tag pass/fail conditions.

5. Synthesize database schema and migration plan
    - Generate relational schema definitions aligned with the domain model and access control policy, including indexes for /pub caching checks and monotonic version stamps.
    - Annotate PII fields with explicit protections and data retention guidance.
    - Review for compatibility with existing DB changes in the repository.

6. Implement storage and query logic
    - Generate SQL/ORM query implementations for selected schema.
    - Generate comprehensive tests for equivalence w/ stub implementation and user permission expectations.
    - Build a targeted test set for concurrent updates, DB migration idempotence, and no negative permission dependency.

7. Finalize user-facing and infrastructure flow
    - Produce JS client-side domain object caching and merge semantics for optimistic updates, with fallback for stale data warnings.
    - Generate CSRF/secret import flow as described in CDN section.
    - Have AI produce a security audit checklist and human validation steps.

8. Review and iterate
    - After each major step, ensure at least one human review of AI outputs for correctness and undesirable auto-optimization.
    - Include a formal verification of unknown-handling semantics in delta merging (human-approved assertions).

!!! caution
    Keep all generated code and tests human-readable and small enough to review in one sitting (e.g., < 100 LOC per module). Large generated artifacts should be split and reviewed incrementally.
...
9. Allocate planning checkpoints for each major risk area
    - XSS and SQL injection risk assessment
    - module/sub-library boundaries and reusable core module structure
    - localization extraction, translation workflow, and translation security guardrails
    - test strategy (unit/integration/end-to-end) with explicit gap identification
    - federated auth and third-party publish action contract requirements

10. Define project planning artifacts
    - clear acceptance criteria for each plan area
    - human review checkpoints
    - a small set of prototyped tasks with estimates for manageable delivery slices

!!! note
    The intent here is to avoid baking tentative, detailed implementation text in the spec at this stage. The full implementation details should be developed in follow-on design docs and tickets as the project progresses.
